...
describe('capitalScoreFromAscent — adapter modello app', () => {
  it('onsight senza montaggio = 1690 (scelta utente)', () =>
    expect(capitalScoreFromAscent({ grade: '7a', ascent_style: 'onsight' })).toBe(1690))
  it('onsight montando (draws_mode placed_by_user) = 1700', () =>
    expect(
      capitalScoreFromAscent({ grade: '7a', ascent_style: 'onsight', draws_mode: 'placed_by_user' }),
    ).toBe(1700))
  it('onsight montando (1700) NON supera 7b+ redpoint (1700)', () =>
    expect(
      capitalScoreFromAscent({ grade: '7a', ascent_style: 'onsight', draws_mode: 'placed_by_user' })!,
    ).toBeLessThanOrEqual(calculateCapitalScore({ grade: '7b+', attemptType: 'redpoint' })!))
